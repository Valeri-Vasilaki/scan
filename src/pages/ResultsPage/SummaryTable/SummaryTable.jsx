...
const SummaryTable = ({ searchData, isLoading, isError }) => {
  ...
  return (
    <div className={styles.summaryTable}>
      ...
      <div className={styles.summaryTable__titleBlock}>
        Найдено данных: {totalDataCount}
      </div>
      <div className={styles.summaryTable__tableAndArrows}>
        <button
          className={styles.summaryTable__scrollBtn}
            ...
            {isLoading ? (
              <div className={styles.summaryTable__loadingData}>
                <img
                  src={loading_icon}
                  alt="Loading"
                  className={styles.summaryTable__loadingData_summary}
                />
                <p className={styles.summaryTable__loadingData_sign}>
                  Загружаем данные...
                </p>
              </div>
            ) : isError ? (
              <div className={styles.summaryTable__error}>
                <p className={styles.summaryTable__errorMessage}>
                ...
                {combinedData.map((item, index) => (
                  <React.Fragment key={index}>
                    <div className={styles.summaryTable__data_row}>
                      <div className={styles.summaryTable__data_cell}>
                        {item.period}
                      </div>
                      <div className={styles.summaryTable__data_cell}>
                        {item.total}
                    ...
                    {index < combinedData.length - 1 && (
                      <div className={styles.summaryTable__divider}></div>
                    )}
                  </React.Fragment>
                ))}
              </div>
